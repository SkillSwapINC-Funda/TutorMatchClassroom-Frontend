...
const ClassroomChat: React.FC<ClassroomChatProps> = ({ classroomId }) => {
  ...
  if (isLoading) {
    ...
    <div className="h-full flex flex-col lg:flex-row gap-2 lg:gap-4">
      {/* Chat principal - Ocupa toda la altura disponible */}
      <div className="flex-1 flex flex-col bg-dark-card overflow-hidden rounded-lg shadow-lg min-h-0">
        {/* Header del chat - Más compacto en mobile */}
        ...
        {/* Mensajes - Optimizado para mobile */}
        <div className="flex-1 min-h-0 overflow-y-auto p-2 sm:p-4 space-y-2 sm:space-y-4 scroll-smooth scrollbar-thin">
          {messages.length === 0 ? (
        ...
        {/* Input para nuevos mensajes - Siempre fijo en la parte inferior */}
